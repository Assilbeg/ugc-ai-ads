...
export default async function DashboardPage({
  ...
  const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
    draft: { label: tStatus('draft'), color: 'text-amber-600', bg: 'bg-amber-500/10' },
    generating: { label: tStatus('generating'), color: 'text-blue-600', bg: 'bg-blue-500/10' },
    assembling: { label: tStatus('assembling'), color: 'text-purple-600', bg: 'bg-purple-500/10' },
    completed: { label: tStatus('completed'), color: 'text-emerald-600', bg: 'bg-emerald-500/10' },
    failed: { label: tStatus('failed'), color: 'text-red-600', bg: 'bg-red-500/10' },
  }
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 p-8 md:p-10">
        ...
        <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <span className="text-sm font-medium text-violet-300">{t('heroBadge')}</span>
            </div>
        ...
        {totalCount > 0 && (
          <div className="relative flex items-center gap-8 mt-8 pt-6 border-t border-white/10">
            <div>
              ...
              <div className="text-sm text-zinc-400">{t('statsCampaigns')}</div>
            </div>
            <div className="w-px h-10 bg-white/10" />
            <div>
              <div className="text-3xl font-bold text-emerald-400">{completedCount}</div>
              <div className="text-sm text-zinc-400">{t('statsCompleted')}</div>
            </div>
            <div className="w-px h-10 bg-white/10" />
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-sm text-zinc-400">{t('completionRate')}</div>
              </div>
            </div>
          </div>
        )}
